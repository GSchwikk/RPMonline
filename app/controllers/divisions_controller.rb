class DivisionsController < ApplicationController
  load_and_authorize_resource param_method: :division_params
  before_action :set_division, only: [:show, :edit, :update, :destroy]
  before_action :set_organisation, only: [:new, :create, :destroy] 
  ...
  def show
    @meetings = Meeting.where(division_id: @division.id)
  end
  ...
  def create
    ...
    @division.organisation_id = @organisation.id
    @division.user_id = current_user.id 
  ...
  def destroy
    @division.destroy
    respond_to do |format|
      format.html { redirect_to :back, notice: 'Division was successfully destroyed.' }
